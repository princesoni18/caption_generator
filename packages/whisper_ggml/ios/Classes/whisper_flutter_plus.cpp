...
void custom_progress_callback(struct whisper_context * ctx, struct whisper_state * state, int progress, void * user_data) {
...
json transcribe(json jsonBody)
{
    ...
    if (jsonBody.contains("initial_prompt") && !jsonBody["initial_prompt"].is_null()) {
        params.prompt = jsonBody["initial_prompt"];
    }
    std::string progress_path = "";
    if (jsonBody.contains("progress_path") && !jsonBody["progress_path"].is_null()) {
        progress_path = jsonBody["progress_path"];
    }
    ...
    // run the inference
    {
        ...
        if (!progress_path.empty()) {
            wparams.progress_callback = custom_progress_callback;
            wparams.progress_callback_user_data = (void*)progress_path.c_str();
        }
